...
//   setenv("TMPDIR",QDir::tempPath().toUtf8().constData(),1);
  ...
  qDebug() << "TMPDIR: " << String::fromUtf8(getenv("TMPDIR"));
...
//    int e=jedo((char *)"script0=: [: 3 : '0!:1 y [ 4!:55<''y''' ]&.:>");
    ...
    if (!e) {
      ...
      if (e) {
        qDebug() << "script0 error:" << String::number(e);
      }
    } else {
  ...
  *q=0;
  ...
  strcat(input,"'");
#ifdef QT_OS_ANDROID
  qDebug() << "j first line: " << String::fromUtf8(input);
#endif
  r=jedo(input);
  if (r) {
    qDebug() << "j first line error: " << String::number(r);
  }
  free(input);
  return r;
}
...
// return success
boolean doriv(String s,I**v,I*len)
{
  String t;
  if (!jt) return false;
  A r=dora(s);
...
// ---------------------------------------------------------------------
// set String in J
void sets(String name, String s)
{
...
//  hdr[0]=(4==n) ? 225 : 227;
  ...
  memcpy(buf,hdr,hlen);
  memcpy(buf+hlen,s.Util.c_str(),slen);
