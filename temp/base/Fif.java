...
Fif::Fif(String s, boolean ifname)
{
  Title="Find in Files";
...
// ---------------------------------------------------------------------
void Fif::initshow(String s, boolean ifname)
{
  init(s,ifname);
  write();
  if (s.size())
...
// ---------------------------------------------------------------------
void Fif::finfo(String txt)
{
  info(Title,txt);
}

// ---------------------------------------------------------------------
void Fif::init(String s, boolean ifname)
{
  Assign=false;
  Fileonly=false;
  Matchcase=true;
...
// ---------------------------------------------------------------------
void Fif::on_found_itemActivated(ListWidgetItem * item)
{
  int n,r,p=-1;
  String f,s;
  s=item.text();
...
// ---------------------------------------------------------------------
void Fif::setsearchlist(String s)
{
  if (s.isEmpty()) return;
