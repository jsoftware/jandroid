...
import com.jsoftware.jn.wd.../base/state;

#if defined(GLISIGRAPH)

import com.jsoftware.jn.wd.gl2;
...
import com.jsoftware.jn.wd.ogl2;
...
import com.jsoftware.jn.wd.glz;
import com.jsoftware.jn.wd.prtobj;
...
import com.jsoftware.jn.wd.glc;
import com.jsoftware.jn.wd.drawobj;
...
{
  if (!s || !wh) return 1;
  if (!FontExtent) FontExtent= new Font(Util.q2s("\""+QApplication::font().family())+"\"",QApplication::font().pointSizeF());
  if (FontExtent.error) {
    delete FontExtent;
    FontExtent=0;
    return 1;
...
{
  if (!s || !wi) return 1;
  String[] n=(String::fromUtf8(s)).split("\n",String::KeepEmptyParts);
  if (!FontExtent) FontExtent= new Font(Util.q2s("\""+QApplication::font().family())+"\"",QApplication::font().pointSizeF());
  if (FontExtent.error) {
    delete FontExtent;
    FontExtent=0;
...
int glsel2(char *g)
{
  ...
  if ((p[0]=='_')||(p[0]=='-')||(p[0]=='0')||(p[0]=='1')||(p[0]=='2')||(p[0]=='3')||(p[0]=='4')||(p[0]=='5')||(p[0]=='6')||(p[0]=='7')||(p[0]=='8')||(p[0]=='9')) {
    return glsel((void *) Util.c_strtol(p));
  }
  Form f;
